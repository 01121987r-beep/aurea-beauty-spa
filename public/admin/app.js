...
function getBookingStatusLabel(status) {
  return { confirmed: 'Confermata', pending: 'In attesa', cancelled: 'Annullata', completed: 'Completata' }[status] || status;
}
...
async function loadDashboard() {
  ...
  dom.bookingFilter.innerHTML = '<option value="all">Tutte</option>' + specialists.map((specialist) => `<option value="${specialist.id}">${specialist.name}</option>`).join('');
